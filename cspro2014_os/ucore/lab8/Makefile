...
.PHONY: qemu debug gdb
QEMUFLAGS := -cpu 4Kc -M malta -m 8M
qemu: $(kernel)
	$(V)$(QEMU) $(QEMUFLAGS) -kernel $< -nographic -s

debug: $(kernel)
	$(V)$(QEMU) $(QEMUFLAGS) -kernel $< -nographic -s -S
...
run-%: build-%
	$(V)$(QEMU) $(QEMUFLAGS) -kernel $(kernel) -nographic -s
